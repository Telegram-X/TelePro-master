...
public class SharedMediaQuery {
    ...
    public static void loadMedia(final long uid, final int count, final int max_id, final int type, final boolean fromCache, final int classGuid) {
        ...
        if (fromCache || lower_part == 0) {
            ...
            req.q = "";
            req.peer = MessagesController.getInstance(UserConfig.selectedAccount).getInputPeer(lower_part);
            if (req.peer == null) {
                return;
            }
    ...
    private static void processLoadedMedia(final TLRPC.messages_Messages res, final long uid, int count, int max_id, final int type, final boolean fromCache, final int classGuid, final boolean isChannel, final boolean topReached) {
        ...
        if (fromCache && res.messages.isEmpty() && lower_part != 0) {
            ...
            for (int a = 0; a < res.messages.size(); a++) {
                ...
                objects.add(new MessageObject(UserConfig.selectedAccount ,message, usersDict, true));
            }
            ...
            AndroidUtilities.runOnUIThread(new Runnable() {
                ...
                public void run() {
                    ...
                    MessagesController.getInstance(UserConfig.selectedAccount).putUsers(res.users, fromCache);
                    MessagesController.getInstance(UserConfig.selectedAccount).putChats(res.chats, fromCache);
                    NotificationCenter.getInstance(UserConfig.selectedAccount).postNotificationName(NotificationCenter.mediaDidLoaded, uid, totalCount, objects, classGuid, type, topReached);
                }
            });
        }
    }
        ...
        AndroidUtilities.runOnUIThread(new Runnable() {
            ...
            public void run() {
                ...
                if (fromCache && count == -1 && lower_part != 0) {
                    ...
                    NotificationCenter.getInstance(UserConfig.selectedAccount).postNotificationName(NotificationCenter.mediaCountDidLoaded, uid, (fromCache && count == -1 ? 0 : count), fromCache, type);
                }
            }
        });
    }
    ...
    private static void putMediaCountDatabase(final long uid, final int type, final int count) {
        MessagesStorage.getInstance(UserConfig.selectedAccount).getStorageQueue().postRunnable(new Runnable() {
            @Override
            public void run() {
                try {
                    SQLitePreparedStatement state2 = MessagesStorage.getInstance(UserConfig.selectedAccount).getDatabase().executeFast("REPLACE INTO media_counts_v2 VALUES(?, ?, ?)");
                    state2.requery();
    ...
    private static void getMediaCountDatabase(final long uid, final int type, final int classGuid) {
        MessagesStorage.getInstance(UserConfig.selectedAccount).getStorageQueue().postRunnable(new Runnable() {
            ...
            public void run() {
                try {
                    int count = -1;
                    SQLiteCursor cursor = MessagesStorage.getInstance(UserConfig.selectedAccount).getDatabase().queryFinalized(String.format(Locale.US, "SELECT count FROM media_counts_v2 WHERE uid = %d AND type = %d LIMIT 1", uid, type));
                    if (cursor.next()) {
                    ...
                    if (count == -1 && lower_part == 0) {
                        cursor = MessagesStorage.getInstance(UserConfig.selectedAccount).getDatabase().queryFinalized(String.format(Locale.US, "SELECT COUNT(mid) FROM media_v2 WHERE uid = %d AND type = %d LIMIT 1", uid, type));
                        if (cursor.next()) {
                            count = cursor.intValue(0);
                        }
                        cursor.dispose();
            ...
            public void run() {
                ...
                try {
                    ...
                    if (!usersToLoad.isEmpty()) {
                        MessagesStorage.getInstance(UserConfig.selectedAccount).getUsersInternal(TextUtils.join(",", usersToLoad), res.users);
                    }
                    if (!chatsToLoad.isEmpty()) {
                        MessagesStorage.getInstance(UserConfig.selectedAccount).getChatsInternal(TextUtils.join(",", chatsToLoad), res.chats);
                    }
                    if (res.messages.size() > count) {
                        topReached = false;
                        res.messages.remove(res.messages.size() - 1);
            ...
            public void run() {
                try {
                    if (messages.isEmpty() || topReached) {
                        MessagesStorage.getInstance(UserConfig.selectedAccount).doneHolesInMedia(uid, max_id, type);
                        if (messages.isEmpty()) {
                            return;
                        }
                    }
                    MessagesStorage.getInstance(UserConfig.selectedAccount).getDatabase().beginTransaction();
                    SQLitePreparedStatement state2 = MessagesStorage.getInstance(UserConfig.selectedAccount).getDatabase().executeFast("REPLACE INTO media_v2 VALUES(?, ?, ?, ?, ?)");
                    ...
                    if (!topReached || max_id != 0) {
                        ...
                        if (max_id != 0) {
                            MessagesStorage.getInstance(UserConfig.selectedAccount).closeHolesInMedia(uid, minId, max_id, type);
                        } else {
                            MessagesStorage.getInstance(UserConfig.selectedAccount).closeHolesInMedia(uid, minId, Integer.MAX_VALUE, type);
                        }
                    }
        ...
        MessagesStorage.getInstance(UserConfig.selectedAccount).getStorageQueue().postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    SQLiteCursor cursor;
                    if (lower_id != 0) {
                        cursor = MessagesStorage.getInstance(UserConfig.selectedAccount).getDatabase().queryFinalized(String.format(Locale.US, "SELECT data, mid FROM media_v2 WHERE uid = %d AND mid < %d AND type = %d ORDER BY date DESC, mid DESC LIMIT 1000", uid, max_id, MEDIA_MUSIC));
                    } else {
                        cursor = MessagesStorage.getInstance(UserConfig.selectedAccount).getDatabase().queryFinalized(String.format(Locale.US, "SELECT data, mid FROM media_v2 WHERE uid = %d AND mid > %d AND type = %d ORDER BY date DESC, mid DESC LIMIT 1000", uid, max_id, MEDIA_MUSIC));
                    }
                    ...
                    while (cursor.next()) {
                        ...
                        if (data != null) {
                            ...
                            if (MessageObject.isMusicMessage(message)) {
                                message.id = cursor.intValue(1);
                                message.dialog_id = uid;
                                arrayList.add(0, new MessageObject(UserConfig.selectedAccount ,message, false));
                            }
                        }
                    }
                    cursor.dispose();
                ...
                AndroidUtilities.runOnUIThread(new Runnable() {
                    @Override
                    public void run() {
                        NotificationCenter.getInstance(UserConfig.selectedAccount).postNotificationName(NotificationCenter.musicDidLoaded, uid, arrayList);
                    }
                });
            }
        });
