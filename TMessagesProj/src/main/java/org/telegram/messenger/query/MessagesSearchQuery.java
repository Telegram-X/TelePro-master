...
public class MessagesSearchQuery {
    ...
    private static void searchMessagesInChat(String query, final long dialog_id, final long mergeDialogId, final int guid, final int direction, final boolean internal, final TLRPC.User user) {
        ...
        if (mergeReqId != 0) {
            ConnectionsManager.getInstance(UserConfig.selectedAccount).cancelRequest(mergeReqId, true);
            mergeReqId = 0;
        }
        if (query == null) {
            ...
            if (direction == 1) {
                ...
                if (lastReturnedNum < searchResultMessages.size()) {
                    ...
                    NotificationCenter.getInstance(UserConfig.selectedAccount).postNotificationName(NotificationCenter.chatSearchResultsAvailable, guid, messageObject.getId(), getMask(), messageObject.getDialogId(), lastReturnedNum, messagesSearchCount[0] + messagesSearchCount[1]);
                    return;
                } else {
            ...
            NotificationCenter.getInstance(UserConfig.selectedAccount).postNotificationName(NotificationCenter.chatSearchResultsLoading, guid);
        ...
        if (queryWithDialog == dialog_id && firstQuery) {
            if (mergeDialogId != 0) {
                TLRPC.InputPeer inputPeer = MessagesController.getInstance(UserConfig.selectedAccount).getInputPeer((int) mergeDialogId);
                if (inputPeer == null) {
                    return;
                }
                ...
                if (user != null) {
                    req.from_id = MessagesController.getInstance(UserConfig.selectedAccount).getInputUser(user);
                    req.flags |= 1;
                }
                req.filter = new TLRPC.TL_inputMessagesFilterEmpty();
                mergeReqId = ConnectionsManager.getInstance(UserConfig.selectedAccount).sendRequest(req, new RequestDelegate() {
                    @Override
        ...
        final TLRPC.TL_messages_search req = new TLRPC.TL_messages_search();
        req.peer = MessagesController.getInstance(UserConfig.selectedAccount).getInputPeer((int) queryWithDialog);
        if (req.peer == null) {
            return;
        }
        req.limit = 21;
        ...
        reqId = ConnectionsManager.getInstance(UserConfig.selectedAccount).sendRequest(req, new RequestDelegate() {
            @Override
            public void run(final TLObject response, final TLRPC.TL_error error) {
                AndroidUtilities.runOnUIThread(new Runnable() {
                    @Override
                    public void run() {
                        if (currentReqId == lastReqId) {
                            ...
                            if (response != null) {
                                ...
                                if (searchResultMessages.isEmpty()) {
                                    NotificationCenter.getInstance(UserConfig.selectedAccount).postNotificationName(NotificationCenter.chatSearchResultsAvailable, guid, 0, getMask(), (long) 0, 0, 0);
                                } else {
                                    if (added) {
                                        if (lastReturnedNum >= searchResultMessages.size()) {
                                            lastReturnedNum = searchResultMessages.size() - 1;
