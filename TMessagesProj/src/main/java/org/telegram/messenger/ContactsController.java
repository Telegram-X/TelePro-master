...
public class ContactsController {
    ...
    public static int FILTER_TYPE_BLOCKED = 4;
    public static int FILTER_TYPE_BLOCKED_ME = 5;
    public static int FILTER_TYPE_MUTUAL = 1;
    public static int FILTER_TYPE_NOT_MUTUAL = 2;
    public static int FILTER_TYPE_ONLINE = 3;
...
//        query.add(builder.build());
    ...
    public void initOnlineUsersSectionsDict() {
        ...
        for (String str : this.usersSectionsDict.keySet()) {
            ...
            while (it.hasNext()) {
                ...
                TLRPC.User user = MessagesController.getInstance(currentAccount).getUser(Integer.valueOf(tL_contact.user_id));
                String s = LocaleController.formatUserStatus(currentAccount, user);
                if (user != null && (user.id == UserConfig.getInstance(currentAccount).getClientUserId() || s.contains(LocaleController.getString("Online", R.string.Online)))) {
        ...
        Collections.sort(this.onlineSortedUsersSectionsArray, new Comparator() {
            @Override
            public int compare(Object str, Object str2) {

                return ((String) str).charAt(0) == '#' ? 1 : ((String) str2).charAt(0) == '#' ? -1 : ((String) str).compareTo(((String) str2));
    ...
    public HashMap<String, ArrayList<TLRPC.TL_contact>> getUsersMutualSectionsDict(int i) {
        return filterHiddenContacts(this.usersMutualSectionsDict, i);
    }

    public HashMap<String, ArrayList<TLRPC.TL_contact>> getUsersSectionsDict(int i) {
    ...
    public HashMap<String, ArrayList<TLRPC.TL_contact>> filterHiddenContacts(HashMap<String, ArrayList<TLRPC.TL_contact>> hashMap, int i) {
        HashMap<String, ArrayList<TLRPC.TL_contact>> hashMap2 = new HashMap();
        for (String str : hashMap.keySet()) {
            ...
            while (it.hasNext()) {
                TLRPC.TL_contact tL_contact = (TLRPC.TL_contact) it.next();
                if (preferences.getInt("hidden_" +tL_contact.user_id, -1) == -1) {
                    if (i == FILTER_TYPE_ALL || ((i == FILTER_TYPE_MUTUAL && tL_contact.mutual) || (i == FILTER_TYPE_NOT_MUTUAL && !tL_contact.mutual))) {
                        arrayList.add(tL_contact);
                    } else {
                        TLRPC.User user = MessagesController.getInstance(this.currentAccount).getUser(Integer.valueOf(tL_contact.user_id));
                        if (user != null && i == FILTER_TYPE_ONLINE && (user.id == UserConfig.getInstance(this.currentAccount).getClientUserId() || (user.status != null && user.status.expires > ConnectionsManager.getInstance(this.currentAccount).getCurrentTime()))) {
                            arrayList.add(tL_contact);
                        } else if (user != null && i == FILTER_TYPE_BLOCKED_ME) {
    ...
    public ArrayList<String> filterHiddenSectionsArray(ArrayList<String> arrayList, HashMap<String, ArrayList<TLRPC.TL_contact>> hashMap) {
