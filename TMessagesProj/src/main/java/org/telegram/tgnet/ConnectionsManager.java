...
public class ConnectionsManager {
    ...
    public ConnectionsManager(int instance) {
        ...
        init(BuildVars.BUILD_VERSION, TLRPC.LAYER, BuildVars.getAppId(), deviceModel, systemVersion, appVersion, langCode, systemLangCode, configPath, FileLog.getNetworkLogPath(), UserConfig.getInstance(currentAccount).getClientUserId(), enablePushConnection);
    }
    ...
    public int sendRequest(final TLObject object, final RequestDelegate onComplete, final QuickAckDelegate onQuickAck, final WriteToSocketDelegate onWriteToSocket, final int flags, final int datacenterId, final int connetionType, final boolean immediate) {
        ...
        if(sharedPreferences.getBoolean("isPoertMode", false)){
            return 0;
        }
        boolean mode = sharedPreferences.getBoolean("ghost_mode", false)  ;
        boolean isPriview = constant.isPriview;

        if (((object instanceof TLRPC.TL_messages_readHistory) || (object instanceof TLRPC.TL_channels_readHistory)) && (isPriview || mode)) {
            return 0;
        }

        if (((object instanceof TLRPC.TL_messages_setEncryptedTyping) || (object instanceof TLRPC.TL_messages_setTyping) || (object instanceof TLRPC.TL_messages_readHistory) || (object instanceof TLRPC.TL_channels_readHistory) || (object instanceof TLRPC.TL_messages_readEncryptedHistory) || (object instanceof TLRPC.TL_messages_readMessageContents)) && (isPriview || mode)) {
            return 0;
        }

        if ((object instanceof TLRPC.TL_account_updateStatus) && (isPriview || mode)) {
